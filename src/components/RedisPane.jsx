...
export function RedisPane({
  ...
  const renderValue = (val, type, specificCmd) => {
    ...
    const canEditScalar =
      ...
      !!onQuery &&
      !!connectionId &&
      ((cmd.name === "GET" && cmd.args.length >= 1) ||
        (cmd.name === "HGET" && cmd.args.length >= 2));
    ...
    if (type === "json" || typeof val === "object") {
      return (
        <pre className="redis-json-view redis-editable" onDoubleClick={onDbl}>
          {JSON.stringify(val, null, 2)}
        </pre>
      );
    }
    return (
      <div className="redis-raw-value redis-editable" onDoubleClick={onDbl}>
        {String(val)}
      </div>
    );
  };
  ...
  const renderHash = (val) => {
    ...
    const allowHashEdit =
      parsedCmd.name === "HGETALL" &&
      parsedCmd.args.length >= 1 &&
      !loading &&
      !!onQuery &&
      !!connectionId;
        ...
        <table className="results-table redis-kv-table">
          ...
          <tbody>
            {entries.map(([k, v]) => {
              ...
              const isEditing =
                editing?.kind === "hash" && editing?.field === k;
              return (
                <tr key={k}>
                  ...
                  <td className="redis-kv-value">
                    {isEditing ? (
                      ...
                      <div
                        ...
                        title={
                          allowHashEdit
                            ? "Double-click to edit (Enter to save, Shift+Enter for newline)"
                            : ""
                        }
                        onDoubleClick={() => {
  ...
  const renderInlineEditor = () => {
    if (!editing || (editing.kind !== "get" && editing.kind !== "hget"))
      return null;
    const title =
      editing.kind === "get" ? (
        <>
        ...
        <>
          Edit field <span className="mono">{editing.field}</span> in{" "}
          <span className="mono">{editing.key}</span>
        </>
      );
    return (
      <div className="redis-inline-editor-overlay">
        <div className="redis-inline-editor-card">
          ...
          <textarea
            ...
            autoFocus
            onChange={(e) =>
              setEditing((prev) => ({ ...prev, draft: e.target.value }))
            }
            onKeyDown={async (e) => {
              if (e.key === "Escape") {
                e.preventDefault();
              ...
              if (e.key === "Enter" && !e.shiftKey) {
                ...
                try {
                  if (editing.kind === "get") {
                    ...
                    await onQuery?.(cmd, connectionId, "redis");
                    await onQuery?.(
                      `HGET ${quoteRedisArg(editing.key)} ${quoteRedisArg(editing.field)}`,
                      connectionId,
                      "redis",
                    );
                  }
          ...
          />
          <div className="redis-inline-editor-hint">
            Enter to save, Shift+Enter newline, Esc cancel
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="pane redis-section">
      {activeTab.results?.message && (
        <div className="redis-warning">⚠️ {activeTab.results.message}</div>
      )}

      {!activeTab.results && !loading && (
        <div className="empty-state">
          Execute a Redis command to see results
        </div>
      )}

      {activeTab.results?.success === false && !activeTab.results.multi && (
      ...
      {activeTab.results?.multi && (
        <div className="results-table-wrapper">
          <table className="results-table">
            ...
            <tbody>
              {activeTab.results.data.map((res, i) => (
                <tr key={i}>
                  <td
                    className="mono"
      ...
      {result && !activeTab.results?.multi && (
